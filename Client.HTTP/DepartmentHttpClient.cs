...
namespace Client.HTTP
{
    public class DepartmentHttpClient : BaseHttpClient
    {
        ...
        public async Task<bool> AddDepartmentAsync(Department department) =>
            await HttpRequestBoolAsync(async () => await Client.PostAsJsonAsync(AddDepartmentUri(), department),Logger);

        public async Task<IEnumerable<Department>> GetDepartmentsAsync() =>
            await HttpRequestResultAsync<Department>(async () => await Client.GetAsync(GetDepartmentsUri()), Logger);

        public async Task<bool> UpdateDepartmentAsync(int id, string newName) =>
            await HttpRequestBoolAsync(async () => await Client.PutAsJsonAsync(UpdateDepartmentUri(id), new { Name = newName }), Logger);
    }
}
