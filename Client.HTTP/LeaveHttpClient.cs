...
namespace Client.HTTP
{
    public class LeaveHttpClient : BaseHttpClient
    {
        ...
        public async Task<bool> AddLeaveAsync(Leave leave) =>
            await HttpRequestBoolAsync(async () => await Client.PostAsJsonAsync(AddLeaveUri(), leave),Logger);

        public async Task<bool> CancelLeaveAsync(int leaveId) =>
            await HttpRequestBoolAsync(async () => await Client.PutAsync(CancelLeaveUri(leaveId), null),Logger);

        public async Task<IEnumerable<Leave>> GetLeavesByEmployeeAsync(int id) =>
            await HttpRequestResultAsync<Leave>(async () => await Client.GetAsync(GetLeavesByEmployeeUri(id)),Logger);

        public async Task<IEnumerable<Leave>> GetLeavesByDateRangeAsync(DateTime start, DateTime end) =>
            await HttpRequestResultAsync<Leave>(async () => await Client.GetAsync(GetLeavesByDateRangeUri(start, end)),Logger);

        public async Task<IEnumerable<Leave>> GetLeaveBalanceAsync(int employeeId) =>
            await HttpRequestResultAsync<Leave>(async () => await Client.GetAsync(GetLeaveBalanceUri(employeeId)),Logger);
